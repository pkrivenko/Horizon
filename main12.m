...
%         end
        ...
        Pdata = Pdata*k_p_up^(ZS_max);
    else
        ZS_max_neighbors = neighbors_indexes(ZS_max_index,:);
        ZS_min_neighbors = neighbors_indexes(ZS_min_index,:);
        for i = 1:n_neighbors
            index = ZS_max_neighbors(i); Pdata(index) = Pdata(index)*k_p_up^ZS_max;
            index = ZS_min_neighbors(i); Pdata(index) = Pdata(index)*k_p_up^ZS_min;
        end
    end
%    last_sign_s = sign(ZS_max);
...
%             k_b_up = k_b_up/1.3;
%         end
        Qdata = Qdata*k_b_up^(ZB_max/2);
    else
        ZB_max_neighbors = neighbors_indexes(ZB_max_index,:);
        ZB_min_neighbors = neighbors_indexes(ZB_min_index,:);
        for i = 1:n_neighbors
            index = ZB_max_neighbors(i); Qdata(index) = Qdata(index)*k_b_up^(ZB_max/5);
            index = ZB_min_neighbors(i); Qdata(index) = Qdata(index)*k_b_up^(ZB_min/5);
        end
    end
%    last_sign_b = sign(ZB_max);
end
end
...
neighbors_prime = find_neighbors(theta_prime, n_neighborsPQ); % TO_DO: check the size of this - works but looks strange
v = zeros(1,6); s = v; b = v;
for age = 1:6
...
% Returns a row vector contaning indexes of n neighbors of theta in theta_sample.
% TO_DO: check this func and/or its usage. Size of results strange.
function y = find_neighbors(theta, n)
global theta_sample
[y,distances] = knnsearch(theta,theta_sample,'K',n);
end

% Predicts Q for given theta using the sample Qdata
...
% Predicts P for given theta using the sample Pdata
...
function y = Qneighbors(Qdata, neighbors)
...
% Two functions to transform data from log scale to level and back
...
% Predicts aggregate Sprime and Bprime given current state and curent guesses (see global variables below).
...
% Force equilibrium: sum(stock)=1; sum(bond)=0.
% Imlementation
% 1) Scale stock demand accordingly
% 2) If bond demand > supply, scale demand so that it = supply. Otherwize scale supply. 
% This func is used to scale initial theta_sample only. A similar code is copied into body of SBaggregate
