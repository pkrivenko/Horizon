...
global f_phi_13 f_phi_14 betaP betaQ betaV betaS betaB order simulated_grid n_epsd epsd weights_epsd n_theta epsw rho_d si_d Dbar options PHI PHIi beta ga fmintimer s_lb s_ub b_lb b_ub last_sign_s last_sign_b theta_sample n_neighborsPQ Pdata Qdata;
...
f_phi_13 = generatepolynomial3(13,2,0);
f_phi_14 = generatepolynomial3(14,2,0);
...
global f_phi_13 f_phi_14 betaQ betaP betaV betaB betaS epsd rho_d Dbar n_epsd rho_d si_d Dbar weights_epsd options PHI PHIi beta ga fmintimer s_lb s_ub b_lb b_ub theta_sample n_neighborsPQ neighbors Pdata Qdata
...
global f_phi_13 f_phi_14 betaQ betaP betaV betaB betaS epsd rho_d Dbar n_epsd rho_d si_d Dbar weights_epsd PHI PHIi ga theta_sample n_neigborsPQ Pdata Qdata
theta = theta_sample(theta_index,:);
D = theta(13);
...
global f_phi_13 f_phi_14 betaS betaB theta_sample Pdata
theta = theta_sample(theta_index,:);
S = theta(:, 1:6);
B = theta(:, 7:12);
D = theta(:, 13);
...
function y = V(x,age)
global f_phi_13 f_phi_14 betaV ga
if age<=6
    y = Phi(x)*betaV(age,:)';
elseif age==7
    y = u(x(1));
...
global f_phi_13 f_phi_14 order
...
elseif order == 2
    for i = 1:n
        z = x(i,:);
        g = @(myfunction) myfunction(z);
        if dim == 13
            y(i,:) = cellfun(g, f_phi_13);
        else
            y(i,:) = cellfun(g, f_phi_14);
        end
