...
global betaP betaQ betaV betaS betaB order simulated_grid N_epsd epsd weights_epsd n_theta epsw rho_d si_d Dbar options PHI PHIi beta ga fmintimer s_lb s_ub b_lb b_ub last_sign_s last_sign_b;
...
while (true)
    ...
    Pdata = exp(PHI*betaP'); % P_mean = mean(Pdata); P_std = std(Pdata);
    Qdata = exp(PHI*betaP'); % B_mean = mean(Bdata); B_std = std(Bdata);
...
v = -beta*v;
b = (w - P(theta)*s)/(Q(theta)); % later can use all precomputed PHI and PHIi but need to know the observation in theta_sample
end
...
for k = 1:N_epsd
    ...
    wprime = (Pprime+Dprime(k))*s + (w - P(theta)*s)/(Q(theta));
