...
global betaP betaQ betaV betaS betaB order simulated_grid N_epsd epsd weights_epsd n_theta epsw rho_d si_d Dbar options PHI PHIi beta ga fmintimer s_lb s_ub b_lb b_ub;
...
while (true)
    ...
    for age = 6:(-1):1
        for state = 1:n_theta
            [Sdata(state,age) Bdata(state,age) Vdata(state,age)] = Psi(w_sample(state,age), theta_sample(state,:), age);
    ...
    disp('    time      iter      fmintimer'); disp([toc(t_total) iter_v fmintimer]);

    % Update betaQ and betaP
    ZS = sum(Sdata')' - ones(n_theta,1);
    ZB = sum(Bdata')';
    
    [ZS_max, ZS_max_index] = max(ZS);
    [ZS_min, ZS_min_index] = min(ZS);
    [ZB_max, ZB_max_index] = max(ZB);
    [ZB_min, ZB_min_index] = min(ZB);
    
    disp('ZS_min     ZS_max     ZB_min    ZB_max'); disp([ZS_min     ZS_max     ZB_min    ZB_max]);
    
    Pdata = PHI*betaP'; % P_mean = mean(Pdata); P_std = std(Pdata);
    Qdata = PHI*betaP'; % B_mean = mean(Bdata); B_std = std(Bdata);
    ...
    if sign(ZS_max)==sign(ZS_min)
        ...
        for i = 1:n_neighbors
            index = ZS_max_neighbors(i);
            Pdata(index) = Pdata(index)*k_p_up;
            index = ZS_min_neighbors(i);
            Pdata(index) = Pdata(index)*k_p_down;
        end
    end
    
    if sign(ZB_max)==sign(ZB_min)
        Pdata = Pdata*k_b_up^(sign(ZB_max));
    else
        ZB_max_neighbors = neighbors_indexes(ZB_max_index,:);
        ZB_min_neighbors = neighbors_indexes(ZB_min_index,:);
        for i = 1:n_neighbors
            index = ZB_max_neighbors(i);
            Bdata(index) = Bdata(index)*k_b_up;
            index = ZB_min_neighbors(i);
            Bdata(index) = Bdata(index)*k_b_down;
        end
    end
...
function [s b v] = Psi(w, theta, age)
global betaQ betaP betaV betaB betaS epsd rho_d Dbar N_epsd rho_d si_d Dbar weights_epsd options PHI PHIi beta ga fmintimer s_lb s_ub b_lb b_ub
negativeEVprime_s = @(s)(-EVprime(s, w, theta, age));
...
v = -beta*v;
b = (w - Phi(theta)*betaP'*s)/(Phi(theta)*betaQ'); % later can use all precomputed PHI and PHIi but need to know the observation in theta_sample
end

% Expected value function tomorrow for given policy s. All the arguments (incl age) are for today.
function y = EVprime(s, w, theta, age)
global betaQ betaP betaV betaB betaS epsd rho_d Dbar N_epsd rho_d si_d Dbar weights_epsd PHI PHIi ga
