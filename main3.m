...
while (true)
    ...
    for j = 1:N_theta
        [Sdata(j) Bdata(j) Vdata(j)] = Psi(w_sample(j,3), theta_sample(j,:));
        j
        
    end
    
    PHI = Phi(theta_sample);
    PHIi = Phi([w_sample(:,3), theta_sample]); % should be different Phi for aggregate and individual state. Or may be embedded into single Phi (check size of state and choose method).
