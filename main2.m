function []=main()
global betaP betaQ betaV betaS betaB order simulated_grid N_epsd epsd weights_epsd N_theta epsw rho_d si_d Dbar;
...
D_sample = randn(N_theta,1).*(ones(N_theta, 1)*(si_d/(1-rho_d))) + ones(N_theta, 1)*Dbar;
...
betaP = [Dbar/(1-Qbar) zeros(1, n_agg-1)];
betaV = rand(1, n_ind);
...
while (true)
    ...
    for j = 1:N_theta
        [S(j) B(j) V(j)] = Psi(w_sample(j), theta_sample(j,:));
        j
        
        % some loop to update betaV
        % ...
        
    end
    
    PHI = Phi(theta_sample);
    PHIi = Phii([w_sample, theta_sample]); % should be different Phi for aggregate and individual state. Or may be embedded into single Phi (check size of state and choose method).
...
[s, v] = fminunc(negativeEVprime_s,.1);
v = -v;
...
Vprime(k) = - Phi([w thetaprime]) * betaV';
end
...
% Expected value function tomorrow BEFORE optimization over S.
function y = EVprime(x)
global betaV betaP betaQ N_epsd epsd weights_epsd N_theta epsw rho_d si_d Dbar

if simulated_grid
    true;
else
    
    DPrime = exp(rho_d*log(D) + (1-rho_d)*log(Dbar) + epsi_nodes);
...
function [DPrime, weight_nodes, Nnodes] = thetaPrimeGrid(D)
global Dbar rho_d si_d
